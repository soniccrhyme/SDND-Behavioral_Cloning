...
if AWS:
    ...
    model.add(Flatten())
    model.add(Dense(100))
    model.add(Dropout(p = 0.75))
    model.add(Dense(50))
    model.add(Dropout(p = 0.75))
    model.add(Dense(10))
    model.add(Dense(1))
    model.compile(loss = 'mse', optimizer = 'adam')
    model.fit_generator(train_generator, samples_per_epoch = BATCH_SIZE, nb_epoch = 10, validation_data = validation_generator, nb_val_samples = BATCH_SIZE)
